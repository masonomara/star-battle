...
- pregenerate puzzle packs (stored as JSON/binary/SBF)
- daily/weekly/monthly: how to deliver? Downstream decision
- do we hold the seed info, or SBF info and generate the puzzle client side? Or do we hold everything server side, or do users just download the puzzle onto their device when they click it, or is everything light enough to just include in the download?
