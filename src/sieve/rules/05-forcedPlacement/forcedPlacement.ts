...
/**
 * If unknowns === needed stars, places one forced star.
...
function placePartitionedForced(
  ...
  // Skip if placeOneForced would handle this
...
export default function forcedPlacement(
  ...
  // First pass: simple forced placement (unknowns === needed)
  ...
  // Second pass: partitioned forced placement (singleton components)
  for (let row = 0; row < size; row++) {
    const coords: Coord[] = [];
    for (let col = 0; col < size; col++) coords.push([row, col]);
