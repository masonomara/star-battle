...
### SBF Format Reference

```text
Format: {size}x{stars}.{regions}.{metadata}

Example: 10x2.0011223344...s42d7l4c12
...
| User opens daily/weekly  | Fetch single SBF from R2, cache in MMKV     |
...
GET /puzzles/monthly/:month
  ...
  - Serve single SBF string from R2
  - CDN caching: Cache-Control: public, max-age=86400
```
...
import { decodeSBF } from "../../src/sieve/helpers/sbf";
...
export async function handleHint(
  ...
  const sbf = getPuzzleSBF(puzzleId);
  const { board } = decodeSBF(sbf);
